...
#include <iostream>
#include <fstream>
void radial_distribution_Optics(){
   gROOT->Reset();
   gStyle->SetOptStat(0);
...
////Change the following lines for which detectors you want to include////
   string detH[] = {"det173","det28","det177","det176","det31"};
//just downstream of target, ring 5, just befor SAM (previous SAM VD location), SAM location in remoll, SAM location in CAD
   ...
   const int Det[nDet] = {173,28,177,176,31};
   map<int,int> dtM {{173,1},{28,2},{177,3},{176,4},{31,5}};
/////////////////////////////////////////////////////////////////////////

   const double x_min = 0;
   const double x_max = 1000;
   const int nbin = 500;
   const Double_t rmin = 500;//rmin for ds scanner
   const Double_t rmax = 700;//rmax for ds scanner
   const int beam_curr = 65;
...
////Change the following lines as needed////
   ...
   const string target = "LH2";
   const string generator = "beam";
   int beamGen(1);
   const string config = "magOFF";
///////////////////////////////////////////

   TFile* outfile = new TFile(Form("./rootfiles/%s_%s_%s_%s_radial.root",geometry.c_str(),
...
////Change this line for appropriate rootfiles////
       string infile = Form("%s/LH2_beam_magOFF/LH2_beam_magOFF_%d.root",
...
//////////////////////////////////////////////////
   ...
   for(int iDet=0;iDet<nDet;iDet++){
      ...
      c_r_lin[iDet]->SaveAs(Form("./temp/%s_%s_%s_%s_radial_lin.pdf",geometry.c_str(),
   ...
   for(int iDet=0;iDet<nDet;iDet++){
      ...
      c_r_log[iDet]->SaveAs(Form("./temp/%s_%s_%s_%s_radial_log.pdf",geometry.c_str(),
                           target.c_str(),generator.c_str(),config.c_str()));
   }
   ...
   for(int iDet=0;iDet<nDet;iDet++){
      for(int iSp=0;iSp<nSp;iSp++){
         ...
         c_rFB_lin[iSp][iDet]->SaveAs(Form("./temp/%s_%s_%s_%s_radial_%s_%s_lin.pdf",geometry.c_str(),
   ...
   for(int iDet=0;iDet<nDet;iDet++){
      for(int iSp=0;iSp<nSp;iSp++){
         ...
         latex.SetTextColor(colorPzL0[iSp]);
         latex.DrawLatex(0.25,0.75,Form("%s Pz<0",spTit[iSp].c_str()));
         c_rFB_log[iSp][iDet]->SaveAs(Form("./temp/%s_%s_%s_%s_radial_%s_%s_log.pdf",geometry.c_str(),
                    target.c_str(),generator.c_str(),config.c_str(),spH[iSp].c_str(),detH[iDet].c_str()));
      }
   }
//Now combine all pdf files saved in ./temp/ directory and save a single pdf file in ./plots/ directory
   gSystem->Exec(Form("pdfunite ./temp/%s_%s_%s_%s_radial_* ./plots/%s_%s_%s_%s_radial.pdf",
                geometry.c_str(),target.c_str(),generator.c_str(),config.c_str(),geometry.c_str(),
                target.c_str(),generator.c_str(),config.c_str()));
   gSystem->Exec(Form("rm -rf ./temp/./temp/%s_%s_%s_%s_radial_*",geometry.c_str(),target.c_str(),
