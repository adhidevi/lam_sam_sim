...
void rad_trans_rate_dist(){
   ...
   int colorF[nDet] = {1,2,3,4,6};
   int colorB[nDet] = {7,8,9,41,46};

   TH1F* h_forward[nDet];
   TH1F* h_backward[nDet];

   for(int iDet=0;iDet<nDet;iDet++){
      h_forward[iDet] = new TH1F(Form("h_forward[%d]",iDet),Form("%s dist. on %s det plane (beam gen);Radius (mm);hits/%sthrownEvents","Radial","SAM","#"),nbin,0,1500);
      h_backward[iDet] = new TH1F(Form("h_backward[%d]",iDet),Form("%s dist. on %s det plane (beam gen);Radius (mm);hits/%sthrownEvents","Radial","SAM","#"),nbin,0,1500);
   ...
   for(int ientry=0;ientry<nentry;ientry++){
      ...
      for(size_t pk=0;pk<fHit->size();pk++){
         ...
         for(int iDet=0;iDet<nDet;iDet++){
           if(detector==DetNo[iDet] && energy>1 && (hitr>=rmin[iDet] && hitr<=rmax[iDet]) && pid==11){
	    if(hitpz>=0)
   ...
   h_forward[0]->Draw("hist");
   h_backward[0]->Draw("hist same");
   for(int iDet=1;iDet<nDet;iDet++){
      ...
      latex.SetTextColor(colorF[iDet]);
      latex.DrawLatex(0.45,0.90-0.05*iDet,Form("%s(%s):%.3e","IntegralF",intRange.Data(),integralF));
      latex.SetTextColor(colorB[iDet]);
      latex.DrawLatex(0.45,0.70-0.05*iDet,Form("%s(%s):%.3e","IntegralB",intRange.Data(),integralB));
   }
      c_rate_linear->SaveAs("./temp/electron_rate_linear.pdf");
   ...
   for(int iDet=1;iDet<nDet;iDet++){
      ...
      c_rate_log->SaveAs("./temp/electron_rate_log.pdf");

//Now combine all pdf files saved in ./temp/ directory and save a single pdf file in ./plots/ directory
   gSystem->Exec(Form("pdfunite ./temp/electron_*.pdf ./plots/PMTSh_beam_V3_FB_electron_det176.pdf"));
   gSystem->Exec(Form("rm -rf ./temp/electron_*.pdf"));
}
