...
std::map<int,string> snDet  {{179,"Ent"},{180,"Exit"}};
string sDet[nDet] = {"Ent","Exit"};
...
for(Int_t k=0;k<nDet;k++){
 for(Int_t i=0;i<nParticle;i++){
  ...
  h_d_rz[part] = new TH2D(part+"_d_rz",Form("Vertex rz dist. at %s for %s",sDet[k].c_str(),sParticle[i].c_str()),3800,-6000,32000,300,-1500,1500);
  h_d_xy[part] = new TH2D(part+"_d_xy",Form("xy dist. at %s for %s",sDet[k].c_str(),sParticle[i].c_str()),300,-1200,-900,300,-50,250);
  h_d_r[part] = new TH1D(part+"_d_r",Form("R dist. at  %s for %s",sDet[k].c_str(),sParticle[i].c_str()),300,900,1200);
  h_d_th[part] = new TH1D(part+"_d_th",Form("Scatt. Angle at  %s for %s",sDet[k].c_str(),sParticle[i].c_str()),100,0,15.0);
/*
...
cout<<hit.vz<<"\t"<<sqrt(hit.vx*hit.vx+hit.vy*hit.vy)<<endl;
...
TString out=Form("./rootfiles/develop_%s_theta.root",tgt_gen_config.c_str());
