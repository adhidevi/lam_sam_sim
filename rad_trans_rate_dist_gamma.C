//plot radial gamma hit distribution on SAM plane
//
...
void rad_trans_rate_dist_gamma(){
   gROOT->Reset();
   gStyle->SetOptStat(0);
   ...
   for(int iDet=0;iDet<nDet;iDet++){
      h_forward[iDet] = new TH1F(Form("h_forward[%d]",iDet),Form("%s dist. on %s det plane (beam gen);Radius (mm);hits/%sthrownEvents","Gamma","SAM","#"),nbin,0,1500);
      h_backward[iDet] = new TH1F(Form("h_backward[%d]",iDet),Form("%s dist. on %s det plane (beam gen);Radius (mm);hits/%sthrownEvents","Gamma","SAM","#"),nbin,0,1500);
   ...
   for(int ientry=0;ientry<nentry;ientry++){
      ...
      for(size_t pk=0;pk<fHit->size();pk++){
         ...
         for(int iDet=0;iDet<nDet;iDet++){
           if(detector==DetNo[iDet] && (hitr>=rmin[iDet] && hitr<=rmax[iDet]) && pid==22){
	    if(hitpz>=0)
...
//      h_backward[iDet]->Draw("hist same");
      ...
      c_rate_linear->SaveAs("./temp/gamma_linear.pdf");
...
//      h_backward[iDet]->Draw("hist same");
      ...
      c_rate_log->SaveAs("./temp/gamma_log.pdf");

//Now combine all pdf files saved in ./temp/ directory and save a single pdf file in ./plots/ directory
   gSystem->Exec(Form("pdfunite ./temp/gamma_*.pdf ./plots/PMTSh_beam_V3_FB_gamma_allE_det176.pdf"));
   gSystem->Exec(Form("rm -rf ./temp/gamma_*.pdf"));
}
