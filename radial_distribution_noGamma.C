...
void radial_distribution_noGamma(){
   gROOT->Reset();
   gStyle->SetOptStat(0);
   ...
   for(int ifile=0;ifile<nfile;ifile++){
      ...
      for(int ientry=0;ientry<nentry;ientry++){
	    ...
	    mtrid = fHit->at(pk).mtrid;
            ...
            phi = fHit->at(pk).ph;
            if(detector==28 && energy>1000 && hitr>500 && pid!=22){
              h_ring5[ifile]->Fill(hitr,asym*rate);
              h_ring5rate[ifile]->Fill(hitr,rate);
            }
            if(detector==174 && energy>1 && hitr>500 && pid!=22){
...
//Now combine all pdf files saved in ./temp/ directory and save a single pdf file in ./plots/ directory
   gSystem->Exec(Form("pdfunite ./temp/*.pdf ./plots/radial_distribution_noGamma.pdf"));
