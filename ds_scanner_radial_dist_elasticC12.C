...
void ds_scanner_radial_dist_elasticC12(){
   gROOT->Reset();
   gStyle->SetOptStat(0);
   ...
   TString file[] = {"pB_quasielc12_off/pB_quasielc12_off","pB_elasticc12_off/pB_elasticc12_off"};
   int nfile = sizeof(file)/sizeof(*file);
   TString sim[] = {"quasielasticC12 mag off", "elasticC12 mag off"};
...
//        if(fileSplit==1089||fileSplit==1090||fileSplit==1091||fileSplit==1092) continue;
          nfileSplit++;
   ...
   latex.DrawLatex(0.65,0.85,"elasticC12");
...
//Now combine all pdf files saved in ./temp/ directory and save a single pdf file in ./plots/ directory
   gSystem->Exec(Form("pdfunite ./temp/*.pdf ./plots/elC12_electrons_ds_scanner_09282021.pdf"));
