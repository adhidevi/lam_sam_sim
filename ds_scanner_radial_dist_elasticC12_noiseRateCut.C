...
void ds_scanner_radial_dist_elasticC12_noiseRateCut(){
   ...
   for(int fileSplit=1001;fileSplit<=1020;fileSplit++){
       nfileSplit++;
       T->Add(rootfile_dir+Form("run_Optics1_HP/run_Optics1_HP_%d.root",fileSplit));
   }
   ...
   for(int ientry=0;ientry<nentry;ientry++){
      ...
      for(size_t pk=0;pk<fHit->size();pk++){
        ...
        if(detector==176 && energy>1 && hitr>100 && pid==11 && (rate<0.012)){
...
//Now combine all pdf files saved in ./temp/ directory and save a single pdf file in ./plots/ directory
   gSystem->Exec(Form("pdfunite ./temp/*.pdf ./plots/elasticC12_electrons_ds_scanner_09292021.pdf"));
