...
#include <fstream>
    ...
    const string spH[nSp] ={"epi-","epi+","g","n","ee1","pri1"};
...
void mom_dist(){
   ...
   for(int iDet=0;iDet<nDet;iDet++){
      eRate[iSp][iDet] = new TH1D(Form("%s_E_%s",detH[iDet].c_str(),spH[iSp].c_str()),Form("Momentum at PMT region for %s (beam generator);E (MeV);hits/%sthrownEvents",detH[iDet].c_str(),"#"),121,-8,5);
    ...
    for(Long64_t ientry=0;ientry<nentry;ientry++){
       ...
       for(int j=0;j<hit->size();j++){
          ...
          if(hit->at(j).r>=rmin[dt] && hit->at(j).r<=rmax[dt]){
            ...
            if(hit->at(j).p>1 && (hit->at(j).pid==11 || hit->at(j).pid==-11)){
              eRate[4][dt]->Fill(hit->at(j).p,rate);
            }
            if(hit->at(j).p>1 && hit->at(j).trid==1){
              eRate[5][dt]->Fill(hit->at(j).p,rate);
            }
          }
       }
    }
    ...
    c_E[iDet]->SaveAs(Form("./temp/momentum_det%d.pdf",Det[iDet]));
    }
    gSystem->Exec(Form("pdfunite ./temp/momentum_det*.pdf ./plots/PMTSh_beam_mom_det%d_det%d.pdf",Det[0],Det[1]));
    gSystem->Exec(Form("rm -rf ./temp/momentum_det*.pdf"));
