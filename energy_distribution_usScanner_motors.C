...
void energy_distribution_usScanner_motors(){
   gStyle->SetOptStat(0);
   for(int iSp=0;iSp<nSp;iSp++){
    for(int iDet=0;iDet<nDet;iDet++){
      if(beamGen)
      hist_title = Form("Kinetic Energy on %s (%s);E (MeV);hits/%sthrownEvents",detH[iDet].c_str(),tgt_gen_config.c_str(),"#");
      else
      hist_title = Form("Kinetic Energy on %s (%s);E (MeV);rate (GHz)",detH[iDet].c_str(),tgt_gen_config.c_str());
    ...
    outfile = TFile::Open(Form("./rootfiles/usScanner_motors_kinE_%s_test.root",tgt_gen_config.c_str()),"RECREATE");
    int nfile=0;
    Long64_t nentry=0;
    long nTotEv=0;
