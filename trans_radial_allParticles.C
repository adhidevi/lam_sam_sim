...
void trans_radial_allParticles(){
   gROOT->Reset();
   gStyle->SetOptStat(0);
   ...
   TFile* outfile = TFile::Open("outrootfiles/sim_lam_allParticles_LAMrG1013mm.root","RECREATE"); 
   ...
   for(int igenerator=0;igenerator<ngenerator;igenerator++){
      ...
      for(int ientry=0;ientry<nentry;ientry++){
	    ...
	    rate = fRate/1.e9/20.0;
            if(detector==detNum[0] && hitr>1013 && energy>1){
            ...
            if(detector==detNum[1] && hitr>500 && energy>1000){
