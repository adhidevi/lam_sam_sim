...
///Change the following lines for which detectors you want to include////
  string detH[] = {"det173","det174","det175","det28","det177","det176","det31"};
//ust downstream of target, ring 5, just befor SAM (previous SAM VD location), SAM location in remoll, SAM location in CAD
  ...
  const int Det[nDet] = {173,174,175,28,177,176,31};
  map<int,int> dtM {{173,1},{174,2},{175,3},{28,4},{177,5},{176,6},{31,7}};
////////////////////////////////////////////////////////////////////////
...
///Change the following lines as needed////
  ...
  const string plotType = "quick_radialNoCut_200bins";
  int beamGen(1);
//////////////////////////////////////////
  ...
  cout<<Form("Total number of entries: %ld",nTotEv)<<endl;
   ...
   for(int iDet=0;iDet<nDet;iDet++){
      ...
      for(int iSp=0;iSp<nSp;iSp++){
         ...
         if(iSp<3)
           ...
           latex.DrawLatex(0.70,0.85-0.05*(iSp-3),Form("%s",spTit[iSp].c_str()));
      }
   ...
   for(int iDet=0;iDet<nDet;iDet++){
      for(int iSp=0;iSp<nSp;iSp++){
         ...
         latex.SetTextColor(color[iSp]);
         latex.DrawLatex(0.65,0.85,Form("%s all Pz",spTit[iSp].c_str()));
         latex.SetTextColor(colorPzG0[iSp]);
         latex.DrawLatex(0.65,0.80,Form("%s Pz>=0",spTit[iSp].c_str()));
         latex.SetTextColor(colorPzL0[iSp]);
         latex.DrawLatex(0.65,0.75,Form("%s Pz<0",spTit[iSp].c_str()));
