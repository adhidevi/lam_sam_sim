...
void trans_radial_newMacros(int particleID){
   gROOT->Reset();
   gStyle->SetOptStat(0);
   ...
   TString generator[] = {"phBlocker_ee","phBlocker_epel","pB_epinel"};
   ...
   TFile* outfile = TFile::Open(Form("outrootfiles/sim_lam_ring5_sam_PID%d_newMacros.root",particleID),"RECREATE"); 
   ...
   for(int igenerator=0;igenerator<ngenerator;igenerator++){
     for(int idet=0;idet<ndet;idet++){
      int nbin2D = 400; int nbin1D = 500; int min2D = -2000;
      int max2D = 2000; int min1D = 500; int max1D = 1500;
      if(idet==2){
        nbin2D = 200; nbin1D = 200; min2D = -150;
        max2D = 150; min1D = 30; max1D = 100;
      }
   ...
   for(int igenerator=0;igenerator<ngenerator;igenerator++){
      ...
      for(int fileSplit=1001;fileSplit<=1100;fileSplit++){
          if(fileSplit==1001 || fileSplit==1026 || fileSplit==1045 ||fileSplit==1098 || fileSplit==1100) continue;
      ...
      for(int ientry=0;ientry<nentry;ientry++){
	    ...
	    rate = fRate/1.e9/20.0;
            ...
            if(detector==detNum[2] && hitr>30 && energy>1 && pid==particleID){
