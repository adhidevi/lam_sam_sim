...
//ust downstream of target, ring 5, just befor SAM (previous SAM VD location), SAM location in remoll, SAM location in CAD
...
//////////////////////////////////////////
  ...
  TFile* outfile = new TFile(Form("./rootfiles/%s_%s_%s_%s_radial_largeBinSize.root",geometry.c_str(),target.c_str(),generator.c_str(),config.c_str()),"recreate");
...
//////////////////////////////////////////////////////////
  ...
  for(int iSp=0;iSp<nSp;iSp++){
   for(int iDet=0;iDet<nDet;iDet++){
     ...
     h_rate[iSp][iDet] = new TH1F(Form("%s_r_%s",detH[iDet].c_str(),spH[iSp].c_str()),Form("Radial dist. on %s plane (%s gen, %s tgt, %s);Radius (mm);hits/#thrownEvents/%.1fmm",detH[iDet].c_str(),generator.c_str(),target.c_str(),config.c_str(),bin_width),nbin,x_min,x_max);
     h_ratePzG0[iSp][iDet] = new TH1F(Form("%s_rPzG0_%s",detH[iDet].c_str(),spH[iSp].c_str()),Form("Radial dist. on %s plane pz>=0 (%s gen, %s tgt, %s);Radius (mm);hits/#thrownEvents/%.1fmm",detH[iDet].c_str(),generator.c_str(),target.c_str(),config.c_str(),bin_width),nbin,x_min,x_max);
     h_ratePzL0[iSp][iDet] = new TH1F(Form("%s_rPzL0_%s",detH[iDet].c_str(),spH[iSp].c_str()),Form("Radial dist. on %s plane pz<0 (%s gen, %s tgt, %s);Radius (mm);hits/#thrownEvents/%.1fmm",detH[iDet].c_str(),generator.c_str(),target.c_str(),config.c_str(),bin_width),nbin,x_min,x_max);
...
///Change this line for appropriate rootfiles////
    string infile = Form("%s/Optics1_beam_magOFF_V4/Optics1_beam_magOFF_V4_%d.root",rootfile_dir.Data(),ifile);
//////////////////////////////////////////////
  ...
  cout<<Form("Total number of entries: %ld",nTotEv)<<endl;
   ...
   for(int iDet=0;iDet<nDet;iDet++){
      ...
      c_r_lin[iDet]->SaveAs(Form("./temp/%s_%s_%s_%s_quick_radial_smallBinSize_lin.pdf",geometry.c_str(),target.c_str(),generator.c_str(),config.c_str()));
    }
   ...
   for(int iDet=0;iDet<nDet;iDet++){
      ...
      c_r_log[iDet]->SaveAs(Form("./temp/%s_%s_%s_%s_quick_radial_smallBinSize_log.pdf",geometry.c_str(),target.c_str(),generator.c_str(),config.c_str()));
   }
   ...
   for(int iDet=0;iDet<nDet;iDet++){
      for(int iSp=0;iSp<nSp;iSp++){
         ...
         c_rFB_lin[iSp][iDet]->SaveAs(Form("./temp/%s_%s_%s_%s_quick_radial_smallBinSize_%s_%s_lin.pdf",geometry.c_str(),target.c_str(),generator.c_str(),config.c_str(),spH[iSp].c_str(),detH[iDet].c_str()));
      }
    }
   ...
   for(int iDet=0;iDet<nDet;iDet++){
      for(int iSp=0;iSp<nSp;iSp++){
         ...
         latex.SetTextColor(colorPzL0[iSp]);
         latex.DrawLatex(0.25,0.75,Form("%s Pz<0",spTit[iSp].c_str()));
         c_rFB_log[iSp][iDet]->SaveAs(Form("./temp/%s_%s_%s_%s_quick_radial_smallBinSize_%s_%s_log.pdf",geometry.c_str(),target.c_str(),generator.c_str(),config.c_str(),spH[iSp].c_str(),detH[iDet].c_str()));
      }
   }
//Now combine all pdf files saved in ./temp/ directory and save a single pdf file in ./plots/ directory
   gSystem->Exec(Form("pdfunite ./temp/%s_%s_%s_%s_quick_radial_smallBinSize_* ./plots/%s_%s_%s_%s_quick_radial.pdf",geometry.c_str(),target.c_str(),generator.c_str(),config.c_str(),geometry.c_str(),target.c_str(),generator.c_str(),config.c_str()));
   gSystem->Exec(Form("rm -rf ./temp/%s_%s_%s_%s_quick_radial_smallBinSize_*",geometry.c_str(),target.c_str(),generator.c_str(),config.c_str()));
