...
#include <iostream>
#include <fstream>
void radial_distribution_manyDet_largeBinSize(){
   gROOT->Reset();
   gStyle->SetOptStat(0);
...
///////////////////////////////////////////
   ...
   TFile* outfile = new TFile(Form("./rootfiles/%s_%s_%s_%s_radial_largeBinSize.root",geometry.c_str(),
...
//////////////////////////////////////////////////
   ...
   for(int iDet=0;iDet<nDet;iDet++){
      ...
      c_r_lin[iDet]->SaveAs(Form("./temp/%s_%s_%s_%s_radial_largeBinSize_lin.pdf",geometry.c_str(),
   ...
   for(int iDet=0;iDet<nDet;iDet++){
      ...
      c_r_log[iDet]->SaveAs(Form("./temp/%s_%s_%s_%s_radial_largeBinSize_log.pdf",geometry.c_str(),
                           target.c_str(),generator.c_str(),config.c_str()));
   }
   ...
   for(int iDet=0;iDet<nDet;iDet++){
      for(int iSp=0;iSp<nSp;iSp++){
         ...
         c_rFB_lin[iSp][iDet]->SaveAs(Form("./temp/%s_%s_%s_%s_radial_largeBinSize_%s_%s_lin.pdf",geometry.c_str(),
   ...
   for(int iDet=0;iDet<nDet;iDet++){
      for(int iSp=0;iSp<nSp;iSp++){
         ...
         latex.SetTextColor(colorPzL0[iSp]);
         latex.DrawLatex(0.25,0.75,Form("%s Pz<0",spTit[iSp].c_str()));
         c_rFB_log[iSp][iDet]->SaveAs(Form("./temp/%s_%s_%s_%s_radial_largeBinSize_%s_%s_log.pdf",geometry.c_str(),
                    target.c_str(),generator.c_str(),config.c_str(),spH[iSp].c_str(),detH[iDet].c_str()));
      }
   }
//Now combine all pdf files saved in ./temp/ directory and save a single pdf file in ./plots/ directory
   gSystem->Exec(Form("pdfunite ./temp/%s_%s_%s_%s_radial_largeBinSize_* ./plots/%s_%s_%s_%s_radial_largeBinSize.pdf",
                geometry.c_str(),target.c_str(),generator.c_str(),config.c_str(),geometry.c_str(),
                target.c_str(),generator.c_str(),config.c_str()));
   gSystem->Exec(Form("rm -rf ./temp/%s_%s_%s_%s_radial_largeBinSize_*",geometry.c_str(),target.c_str(),
